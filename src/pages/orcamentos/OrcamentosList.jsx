...
import { Plus, Eye, Edit, Trash2, Download, Search } from 'lucide-react'
...
const OrcamentosList = () => {
  ...
  useEffect(() => {
    const t = setTimeout(() => setAppliedSearch(searchTerm.trim()), 350)
    return () => clearTimeout(t)
  }, [searchTerm])

  useEffect(() => {
    setPage(1)
  }, [empresaAtual?.id, appliedSearch])
  ...
  const loadOrcamentos = async () => {
    try {
      ...
      setTotalCount(count)
      setTotalPages(count > 0 ? Math.ceil(count / API_PAGE_SIZE) : 1)
    } catch (error) {
  ...
  const rangeStart =
    totalCount === 0 ? 0 : (page - 1) * API_PAGE_SIZE + 1
  const rangeEnd = Math.min(page * API_PAGE_SIZE, totalCount)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        {/* Barra de filtros: exibir + busca (largura total) + total */}
        <div className="flex flex-col gap-4 pb-4 border-b border-secondary-200">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4 w-full min-w-0">
            <div className="flex items-center gap-3 shrink-0 w-full sm:w-auto">
              ...
              <div className="flex flex-1 sm:flex-initial border border-secondary-200 rounded-lg p-0.5 h-10 min-h-[2.5rem] w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => {
            ...
            <div className="relative flex-1 min-w-0 w-full">
              ...
              <Input
                ...
                placeholder="Nº, cliente, CNPJ, nome do status…"
            ...
            {!loading && !loadError && (
              <p className="text-sm text-secondary-500 shrink-0 whitespace-nowrap sm:text-right">
          ...
          <>
                ...
                <tbody>
                  {orcamentos.map((orcamento) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">{getStatusBadge(orcamento)}</td>
            ...
            <div className="md:hidden space-y-4">
              {orcamentos.map((orcamento) => (
                <Card
                  key={orcamento.id}
                  className="p-4 cursor-pointer hover:bg-secondary-50/60 transition-colors"
                  padding="none"
                  onClick={() => navigate(`/orcamentos/${orcamento.id}`)}
                >
                  <div className="space-y-3 p-4">
                    ...
                    <div
                      className="flex items-center justify-end gap-2 pt-2 border-t border-secondary-200"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <button
                        type="button"
            ...
            {/* Paginação: sempre visível com resultados (evita surpresa com muitos registros) */}
            {totalCount > 0 && (
